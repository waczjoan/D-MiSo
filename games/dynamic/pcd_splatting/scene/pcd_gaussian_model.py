...
class PcdGaussianModel(GaussianModel):
    ...
    def get_attached_scales(self, scales):
        ...
        attached_scales = self.scaling_activation(self.attached_scale) * scales.unsqueeze(1).expand(-1, self.num_splats,
                                                                                                    -1).reshape(-1, 3)
    ...
    def setup_attached_gauss(self, num_splats=500, num_gauss=2000, alpha_lr=1e-3):
        ...
        features_dc = self._features_dc.unsqueeze(1).expand(-1, num_splats, -1, -1).flatten(start_dim=0,
                                                                                            end_dim=1).clone()
        features_rest = self._features_rest.unsqueeze(1).expand(-1, num_splats, -1, -1).flatten(start_dim=0,
                                                                                                end_dim=1).clone()
    ...
    def training_setup(self, training_args):
        ...
        self.pseudomesh_scheduler_args = get_expon_lr_func(
            lr_init=training_args.pseudomesh_lr_init * self.spatial_lr_scale,
